...
1. [GitHub Releases](https://github.com/DAWNCR0W/SeaLegs/releases)에서
   `SeaLegs-0.2.0-arm64.dmg`와 `SHA256SUMS.txt`를 다운로드합니다.
...
- 게임 창 이동과 크기 변경을 따라가거나, 게임이 실행 중인 화면 또는 모든
  화면을 대상으로 선택할 수 있는 투명 클릭 통과 오버레이
...
- 게임 설정 체크리스트
- 캘리브레이션과 진단 내보내기
- 프로필별 중앙 점과 조준선 위치 조절
- 충돌 내용을 확인할 수 있는 `.sealegsprofile` 가져오기와 내보내기
- 개인정보를 제외한 호환성 확인과 복사 가능한 지원 리포트
- macOS 로그인 항목을 통한 선택적 로그인 시 실행
...
8. Settings > General에서 `Game Window`, `Active Game Display`, `All Displays` 중 하나를 선택합니다.
...
기본 산출물은 `dist/v0.2.0/`에 생성됩니다. 스크립트는 최종 파일을
